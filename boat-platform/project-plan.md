...
| R11 | Plugin ABI breakage | Medium | High | ABI v9 checked at dlopen (plugin_manager.cpp:113). Mismatched .so is rejected, which prevents silent breakage but leaves the user with a hard error and no fallback path (v7 fallbacks were removed in the v8 cleanup). |
