...
scenario_app = typer.Typer(help="Scenario management (create, get, list, delete, validate).")


@scenario_app.command("create")
def create_scenario(
    ctx: typer.Context,
    file: Path = typer.Option(..., "--file", help="Path to a scenario JSON file. "
                               "Its top-level \"id\"/\"name\" fields are used if present, "
                               "otherwise the filename (without extension) is used for both."),
) -> None:
    """Create a scenario from a local JSON file, uploading its content to the gateway."""
...
def get_scenario(
    ctx: typer.Context,
    scenario_id: str = typer.Argument(..., help="Scenario ID (see `boat scenario list`)."),
) -> None:
    """Fetch a scenario's full content by ID."""
...
def list_scenarios(ctx: typer.Context) -> None:
    """List every scenario stored on the gateway."""
...
def validate_scenario(
    ctx: typer.Context,
    file: Path = typer.Option(..., "--file", help="Path to a scenario JSON file to validate."),
) -> None:
    """Validate a scenario file's content without storing it, reporting any issues found."""
    content = file.read_text(encoding="utf-8")
