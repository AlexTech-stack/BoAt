...
def configure_route(
    ...
    src_port: Annotated[int, typer.Option("--src-port", help="UDP source port (UDP/IP mode only, i.e. --src-ip set).")] = 0,
    dst_port: Annotated[int, typer.Option("--dst-port", help="UDP destination port (UDP/IP mode only).")] = 0,
    ttl:      Annotated[int, typer.Option("--ttl", help="IP TTL for UDP/IP mode.")] = 64,
    vlan_id:  Annotated[int, typer.Option("--vlan", help="802.1Q VLAN ID for Ethernet transport (0 = untagged).")] = 0,
...
def configure_container(
    ...
    src_port: Annotated[int,  typer.Option("--src-port", help="UDP source port. Overrides database value.")] = 0,
    dst_port: Annotated[int,  typer.Option("--dst-port", help="UDP destination port. Overrides database value.")] = 0,
    ttl:      Annotated[int,  typer.Option("--ttl", help="IP TTL. Overrides database value (falls back to 64 if neither is set).")]      = 0,
    vlan_id:  Annotated[int,  typer.Option("--vlan", help="802.1Q VLAN ID. Overrides database value (0 = untagged).")]     = 0,
    db:       Annotated[str,  typer.Option("--db",       help="PDU database JSON file.", autocompletion=complete_json_file)] = "pdu_db.json",
) -> None:
