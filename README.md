...
- **Plugin SDK** — C ABI **v8** plugin interface built around a single unified `BoatFrame` type (CAN/CAN-FD/Ethernet/PDU/TCP). Plugins implement `on_tick`, `on_frame`, `set_frame_publisher`, and `declared_buses`; the gateway dispatches every frame through `PluginManager::DispatchFrame()`. `BOAT_CAN_FLAG_SELF_SENT` (0x08) / `BOAT_ETH_FLAG_SELF_SENT` (0x01) tag internally-dispatched frames to prevent self-loop. Load `.so` plugins at runtime with JSON config (`plugin.so?{...}`). As of v8 even PDU routing is a plugin (`pdu_router.so`). Built-in plugins: PduRouter, FrameForwarder, CanIo, CAN-TP (ISO 15765-2), TCP, SOME/IP, vehicle dynamics, sensor simulation, network sim, CAN responder.
