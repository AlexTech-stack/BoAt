...
class EthInNode(Node):
    def start(self, client: BoAtClient, graph: "Graph") -> None:
        ...
        expected_et = parse_value(et_flt) if et_flt else None
        def _run() -> None:
            while True:
                try:
                    stream = client.ethernet.SubscribeFrames(
                        ethernet_pb2.SubscribeEthernetFramesRequest(
                            iface=iface, ethertype=0
                        )
                    )
                    for frame in stream:
                        if expected_et is not None and frame.ethertype != expected_et:
                            continue
                        msg = {
                            "_type": "eth_frame",
                            "payload": {
...
class CanOutNode(Node):
    ...
    def process(self, msg: dict) -> None:
        ...
        if t and t not in ("can_frame", "any"):
            ...
            return None
        raw_iface = self.config.get("iface")
        iface  = raw_iface if raw_iface is not None else (get_field(msg, "payload.iface") or "")
        raw_can_id = self.config.get("can_id")
        can_id = parse_value(raw_can_id) if raw_can_id is not None else None
        if can_id is None:
            can_id = get_field(msg, "payload.can_id") or 0
        raw_data = self.config.get("data")
        if raw_data is not None:
            data = _hex_bytes(raw_data)
        else:
            data = _hex_bytes(get_field(msg, "payload.data") or b"")
        data   = bytes(data[:8])
...
class EthOutNode(Node):
    ...
    def process(self, msg: dict) -> None:
        ...
        raw_iface = self.config.get("iface")
        iface     = raw_iface if raw_iface is not None else (get_field(msg, "payload.iface") or "")
        raw_et       = self.config.get("ethertype")
        parsed_et    = parse_value(raw_et) if raw_et is not None else None
        if parsed_et is not None:
            ethertype = int(parsed_et)
        else:
            ethertype = int(get_field(msg, "payload.ethertype") or 0x0800)
        raw_data = self.config.get("data")
        if raw_data is not None:
            data = _hex_bytes(raw_data)
        else:
        ...
        try:
            if sig_type == "number":   sig.number_value = float(value or 0)
                ...
                if isinstance(value, bool):
                    ...
                    sig.bool_value = value.lower() in ("true", "1", "yes")
                else:
                    sig.bool_value = bool(value)
...
class CanToBytesNode(Node):
    ...
    def process(self, msg: dict) -> dict | None:
        ...
        p      = msg.get("payload", {})
        raw_can_id = p.get("can_id")
        can_id = int(raw_can_id) if raw_can_id is not None else 0
        raw_data = p.get("data")
        data = bytes(raw_data) if raw_data is not None else b""
        raw_dlc = p.get("dlc")
        dlc = int(raw_dlc) if raw_dlc is not None else len(data)
