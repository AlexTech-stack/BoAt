...
4. **Plugin lifetime is pinned, not assumed.** `Unload()` is reachable from gRPC and can race the tick thread. `TickAll`/`DispatchFrame` must release the mutex before calling into a `.so` (plugins call back into the host), so they **pin** `PluginManager::busy_count_` atomically with their snapshot; `Unload()` erases from both maps, then waits for the count to drain before `destroy_fn` + `dlclose`. This is what makes plugin.h's *"Host guarantees no concurrent callbacks after return"* true. From anything reachable off a gRPC handler use `AcquireService<T>()` (returns a pinning `ServiceRef<T>`), **not** bare `FindService()`. Keep refs short-lived — one held across a streaming RPC blocks `Unload` for the life of the stream, trading the use-after-free for a hang; the streaming RPCs store iface *names* and re-acquire per use instead.
...
- `BOAT_CONFIG_DUMP` — path to write the **effective config** JSON at startup (see below)
- `BOAT_TIME_SOURCE=realtime|virtual` — selects the `TickTimer` backend (`src/hil/pdu/tick_timer.cpp`). Unset, empty, or unrecognised values all yield real time; only the exact string `virtual` opts in, since a typo must never quietly stop a HIL bench pacing real hardware. **The gateway still passes `kRealTime` explicitly** — a bare gateway paces hardware, so going virtual is a deliberate deployment choice rather than an env default. Now that node plugins and replay share one `TickAuthority`, selecting `virtual` is meaningful (it advances the whole pipeline logically) rather than just spinning one loop.
...
boat config show [-o effective-config.json]  # what the gateway actually resolved
```

**Effective configuration.** Gateway config is env vars resolved in `main()`,
and several knobs used to fall back *silently* — a typo'd `BOAT_NODE_TICK_US`
left you at the 1ms default with nothing on stderr. Resolution now happens once
into `EffectiveConfig` (`src/gateway/grpc_gateway/effective_config.{h,cpp}`),
which records the values used, which env var decided the tick, whether each
interface actually opened, each plugin's verbatim config and load result, and a
`warnings` list. It is written to `BOAT_CONFIG_DUMP` at startup and served by
`DebugService.GetEffectiveConfig` / `boat config show`, so a CI run can commit
